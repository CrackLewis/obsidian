...
- optimize the $(k,\eta)$-core computation for large $k$'s:
	...
	- suppose that $k_0$ is the lowest $k$ such that a $(k,\eta)$-core can be detected. then for all $k>k_0$, no re-computation of $\eta$-deg's is required, since $\Pr[\deg(v)\ge k]\ge \Pr[\deg(v)\ge k_0]\ge \eta$.