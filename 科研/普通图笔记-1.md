...
*Solution 1*: a global search algorithm (named *Global* specifically):  ^9dc3a6
- let $G_0=G$
...
*Solution 2*: a local search algorithm (named *Local* specifically): ^e74f12
...
> - no subgraph $H\subset H'$ exists to satisfy the above prop's.

^11fe7a

a basic sol' to Prob.24:
- steps:
	- sequentially enum' all maximal k-ECCs by varying $k$ from $|V|$ to 1
	- stops when the first k-ECC containing $Q$ is found
- k-ECC enum' algo's:
	- graph decomposition: time $O(|V|\cdot h\cdot l\cdot |E|)$, where $h,l$ are small constants
	- random contraction: time $O(|V|\cdot t\cdot |E|)$, where $t=O(\log_2^2 |V|)$
	- not effn't enough
- improvements: a novel compact index structure
	- construction: based on observation that for any edge $(u,v)\in E_H$, its edge conn'ty $\lambda(u,v)\ge \lambda(H)$, which implies that query can be answered in linear time if every edge's conn'ty is preserved
	- process: determine $\lambda(H)$ by checking conn'ty of vtx' pairs in $Q$, and find $H$ by traversing conn'd edges with conn'ty at least $\lambda(H)$
	- *connectivity graph*: 
		- identical to $G$ in structure, but each edge is assoc'd with its conn'ty in $G$
		- index is generated by calculating minimum spanning tree (MST) on $G_c$, with space cost $O(|V|)$
- an effn't query algo' based on index:
	- computes $\lambda(H)$ using MST and finds the max' SMCS by collecting MST subtrees with conn'ty at least $\lambda(H)$
	- utilize the lowest common ancestor (LCA) to achieve $O(|H_V|)$ time cost

说人话：

斯坦纳最大连接子图问题（问题24）：给定图和查询结点集，返回包含查询点集且连通度最高的子图。
...
#### minimum and minimal SMCS's

prob' with maximal SMCS's: their size is extremely large

minimum SMCS prob' is desc'd as Prob.25:

> [!problem] Prob. 25
> Given an undrct' graph $G$, and a set of query vtx's $Q\subseteq V$, return a subgraph $H(V_H,E_H)$ of $G$, such that:
> - $V_H$ contains $Q$.
> - $\lambda(H)$ is maximized.
> - $|H_V|$ is minimized.

minimal SMCS prob' is desc'd as Prob.26:

> [!problem] Prob. 26
> Given an undrct' graph $G$, and a set of query vtx's $Q\subseteq V$, return a subgraph $H(V_H,E_H)$ of $G$, such that:
> - $V_H$ contains $Q$.
> - $\lambda(H)$ is maximized.
> - no other subgraph $H'\subset H$ exists to satisfy the above prop's.

obviously, a minimum SMCS is also a minimal SMCS, both of which are much smaller than the maximum SMCS.

the minimum SMCS prob' (Prob.25) is APX-hard. it's neither solvable nor approx'-able in polynomial time. so it's more practical to solve Prob.26.

sol's to Prob.26:
- a naive sol': compute the maximum SMCS $G'$ first, and iteratively refine $G'$ to ensure its minimality
	- time consuming in testing the minimality of the SMCS
- *Expand-Refine* framework:
	- steps:
		- compute the Steiner conn'ty of the query vtx's $Q$ (i.e. the maximum $\lambda(H)$)
		- Expand: obtain subgraph $H'$ by local expansion of vtx's starting from $Q$, whose conn'ty $\lambda(H')=\lambda(H)$
		- Refine: remove vtx's based on the dependence of vtx's on their minimal SMCS's
	- time $O(t\cdot h\cdot l\cdot |E|)$, where $t<|H_V|$ and $h,l$ are small const's
	- relaxation of constraints can improve the effn'cy too
- a special algo' when $|Q|=1$: cache the processing info' related to current query

#### discussions

two classes:
- maximum SMCS: effn't but space consuming
- minimum & minimal SMCS: space saving but not effn't enough

future study: k-ECC on attributed graphs

### sec07-CS based on other metrics

#### local modularity-based CS

*boundary-based local modularity*: 

for an undrct' graph $G$ and three vtx' set $V_C,V_U,V_B\subseteq V_G$:
- $V_C$: the known set, containing vtx's in the known proportion of the comm'ty
- $V_U$: the unknown set, containing vtx's adjacent to vtx's in $V_C$
- $V_B$: the boundary set, a subset of $V_C$, containing vtx's having neighbors in $V_U$

local modularity: the local modularity of $V_C$ is:
$$
f(V_C)=\dfrac{I}{T}
$$
where:
- $I$ is the edge number with no end vtx in $V_U$
- $T$ is the edge number with at least one end vtx in $V_B$

rel' btwn' local modularity to a good comm'ty: good comm'ties have sharp boundaries, which implies higher $I$ and lower $T$ and therefore higher $f(V_C)$

a vtx'-at-a-time algo':
- let $V_C=\{q\}$, $V_U=\{v\mid (v,q)\in E_G\}$ initially
- for each step, add to $V_C$ the neighboring vtx' resulting in largest increase of the local modularity
- repeat the prev' step util:
	- either it has agglomerated a given number of vtx's $k$
	- or it has discovered the entire enclosing component
- time $O(k^2d)$, where $d$ denotes the mean deg' and $k$ is the num of vtx's to be explored

*subgraph degree-based local modularity*:

Given a subgraph $C$ of a graph $G$, its:
- in-degree: denoted by $ind(C)$, number of edges within $C$
- out-degree: denoted by $outd(C)$, number of edges connecting $C$ to $G-C$.
- subgraph modularity: $f(C)=ind(C)/outd(C)$.

an algo':
- steps:
	- let $C=\{q\}$, $N=\{v\mid (v,q)\in E_G\}$ 
	- addition step: add vtx's resulting in $f(C)$'s greatest increase from $N$ to $C$, until a certain num' of neighbors have been in $C$
	- deletion step: removes vtx's from $C$ resulting in $f(C)$'s increase but not separating $C$
	- repeat the two prev' steps until no vtx' is added to $C$
- time is similar to boundary-based one

#### query biased density-based CS

vertex weight $r(u)$: becomes higher if $u$ is away from query vtx's
- can use the penalized hitting probability as a metric

query biased vertex weight: reciprocal of $r(u)$, i.e. $\pi(u)=1/r(u)$

*query biased density*: given a graph $S$, its QBD is $\rho(S)=e(S)/\pi(S)$, where:
- $e(S)$: the sum of edge weights
- $\pi(S)$: the sum of query biased weights for vtx's in $S$
	- if $\pi(u)=1$, the query biased density is the graph density $\rho(S)=e(S)/|S|$.

query biased densest subgraph (*QDS*) prob':
- aims to find the densest subgraph containing query vtx's based on query biased weights
- is computationally *intractable* (棘手的)
- has two prob' variants by removing constraints

#### personalized PageRank-based CS

PageRank model: 
- suppose a surfer is traveling on a graph continuously, after every timestamp the surfer goes to a random neighbor
- the PageRank score $r(i)$ of vtx' $i$ is the time ratio when the surfer is staying at vtx' $i$

personalized PageRank (*PPR*) model:
- in PageRank model, all vtx's are equally accessed, but in real life prob's, some vtx's denoted by  might be more interesting
- modification to PageRank: the surfer has a chance of $c$ to jump to $Q$, and $(1-c)$ to a neighbor, adding favor to vtx's in $Q$

formal definition of PPR model: given $G$ and its adjacent matrix $A$, where $A_{i,j}=\dfrac{1}{\deg_G(i)}$ if $(i,j)$ are conn'd. the preference vector $u$  is def'd over the seed vtx's such that $|u|=1$, $u(i)=1/|Q|$ for all $i\in Q$. then the PPR equation is:
$$
v=(1-c)Av+cu
$$
where $c\in (0,1]$ denotes the decay factor (normally $c=0.1$), and $v$ is the PPR vector desc'ing the steady-state distribution of the surfers.

PPR prob' is desc'd as Prob.27:

> [!problem] Prob. 27
> Given a graph $G(V,E)$, a set of query vtx's $Q\subseteq V$, and an int' $k$. Return a set of vtx's $C$, such that:
> - $Q\subseteq C$.
> - $C$ contains $k$ vtx's, whose corresponding values in the PPR vector w.r.t. $Q$ are the highest.

there are some effn't algo's. but the paper skipped those.

#### neighbors expansion-based CS

neighbor expansion method:
- starts from seed vtx's $C=S$, and selects neighbor of $C$ to be joined repeatedly
- selection criteria:
	- neighbor count: the number of vtx's neighbor in $C$
	- juxtaposition count: weights of edges connecting vtx' to $C$
	- neighbor ratio: degree-normalized neighbors of $C$
	- juxtaposition ratio
	- binomial probability: the binomial probability that $v$ is in $C$, given its neighbor count
- stopping rules: utilize validation members to check the accuracy

#### discussions

future study:
- how to apply them to online big graphs
- how to use them on attribute graphs

### sec08-CS systems

two groups in graph processing systems:
- supporting general graph tasks: e.g. GraphX, Pregel
- customized for specific tasks: Expfinder, VIIQ, AutoG

CS systems: C-Explorer, VizCS

#### C-Explorer

simple, online, interactive CS

features:
- CS on simple and kw-based graphs:
	- [[#^9dc3a6|Global]] and [[#^e74f12|Local]]
	- [[#^3c1c92|ACQ]] problem
	- comm'ty detection: CODICIL
- graph visualization
- compare comm'ties by their qualities and statistics
- API functions for other CS/CD algo's to be plugged in

#### VizCS

features:
- triangle-conn'd truss comm'ty model for dyn' graphs
- visual interface
- comm'ty exploration wall
- compare comm'ties by various SOTA algo's and user-defined approaches

### sec09-comparison analysis

#### simple graph CS comparison

representative prob's for each cohesiveness metric:
- k-core: [[#^f4029a|Prob. 1: size-unbounded k-core CS on undirected graph]]
- k-truss: [[#^584101|Prob. 15: triangle-conn'd truss comm'ty (TTC) search]]
- k-clique: [[#^6ae289|Prob. 20: densest clique percolation community (DCPC]]
- k-ECC: [[#^11fe7a|Prob. 24: maximum Steiner maximum-conn'd subgraph (SMCS) search]]

consider a simple undrct' graph $G(V,E)$, where:
- $|V|=n,|E|=m$. 
- the *arboricity* (荫度) of $G$ is $\alpha(G)$, which is often much smaller than $\sqrt{m}$.
- $h,l$ are used to denote small constants.

table desc':
- OA: online algo'
- IA: index-based algo'
- prop's comparison: 4 is the best>3>2>1 is the worst
- $C$ denotes the only comm'ty returned, $C_i$ denotes the i-th comm'ty returned
- OC: supports overlapping comm'ties; DG: supports dynamic graphs
- $s$: avg' size of max' cliques
- $T$: time to enum' all max' cliques
- $L$: the number of max' cliques
- $p$: avg' num of max' cliques a vtx' is contained in
- $Q$: the num of max' cliques containing at least one query vtx'
- $g$: height of the index tree

| metrics        | k-core       | k-truss                       | k-clique             | k-ECC                   |
| -------------- | ------------ | ----------------------------- | -------------------- | ----------------------- |
| OA-query time  | $O(n)$       | $O(m^{1.5})$                  | $O(\log n\cdot sT)$  | $O(hlm)$                |
| OA-scalability | 4            | 2                             | 1                    | 3                       |
| IA-time        | $O(m)$       | $O(m^{1.5})$                  | $O(sLp)$             | $O(\alpha(G)\cdot hlm)$ |
| IA-space       | $O(n)$       | $O(m)$                        | $O(sL)$              | $O(m)$                  |
| IA-scalability | 4            | 3                             | 1                    | 2                       |
| IA-query time  | $O(\|E_C\|)$ | $O(\sum_{i=1}^r \|E_{C_i}\|)$ | $O(g\log(g)\cdot Q)$ | $O(\|E_C\|)$            |
| cohesiveness   | 1            | 3                             | 4                    | 2                       |
| supports OC    | N            | Y                             | Y                    | N                       |
| supports DG    | Y            | Y                             | Y                    | Y                       |

observations:
- scalability and cohesiveness are contrary
- query time in IA are optimal except k-clique sol', i.e. linear to $|E_C|$.
- k-core and k-ECC returns only one comm'ty, while the rest returns multiple comm'ties.
- k-truss IA's are slower in building the index
- k-core IA's query is empirically slower than k-truss IA due to larger comm'ty size
- k-truss OA/IA has the best comm'ty quality in terms of diameter, density and clustering coefficient

#### attributed graph CS comparison

comparison on temporal, profiled, location-based graph CS:
- inoperable bcz' only k-core sol's are studied

comparison on influence value-based graph CS:
- k-core: infl' values are assoc'd with vtx's
- k-truss: infl' values are assoc'd with edges denoting the edge importance
- k-clique: infl' values are assoc'd with edges denoting the vtx' influence possibility
- comparing these sol's are impractical

comparison on kw'-based graph CS:
- two representative studies:
	- ACQ: k-core model, more cohesive but costly
	- ATC: k-truss model, less cohesive but less costly
- comm'ty qualities:
	- ATC is more accurate in searching comm'ties
	- ACQ outperforms ATC in efficiency
...
research directions:
- optimization for query param's: provide users with param suggestions
- more cohesiveness metrics: see 10.2 (P33/40) for details
	- structure cohesiveness
	- attr'-based cohesiveness
- other types of graphs:
	- public-private network: each vtx' $u$ in $G$ is assoc'd with a private network $G_u$ known only to $u$
	- uncertain graph: graph data might be noisy, inexact and inaccurate
	- signed graph: edges carry signs (e.g. friendship or hostility)
	- multi-dimensional graph
	- heterogeneous information network (HIN)
- real big graphs:
	- query algo's on distributed systems: e.g. GraphX
	- I/O effn't query algo's
- online repo's for codes and datasets
