...
- 处理过程：处理过程的具体处理逻辑一般用判定表或判定树来描述。`处理过程描述 = {处理过程名，说明，输入：{数据流}，输出：{数据流}，处理：{简要说明}}`。
...
E-R图转化为关系模型的方法：
- `1:1`联系：可转换为独立的关系模式，也可和任意一端对应的关系模式合并。
- `1:n`联系：可转换为独立的关系模式，也可和`n`端对应的关系模式合并。
- `m:n`联系：转换为一个大的关系模式，与该联系相连的各实体的码以及联系本身的属性均转换为关系的属性，各实体的码组成关系的码或关系码的一部分。

数据模型的优化方法：
- 确定数据依赖、分析各关系模式*属于第几范式*，并不是规范化程度越高的关系就越优，需要考虑具体情况
- 对关系模式进行*必要的分解*，提高数据操作的效率和存储空间的利用率：
	- *水平分解*：把关系的元组分为若干子集合，定义每个子集合为一个子关系
	- *垂直分解*：把关系的属性分解为若干个子集合，形成若干子关系模式

设计用户子模式：如果某些局部应用经常要使用某些很复杂的查询，为了方便用户，可以将这些复杂查询定义为视图，用户每次只对定义好的视图进行查询即可。
...
关系数据库的查询处理分四个阶段：
- 查询分析：对查询语句进行扫描、词法分析、语法分析，判断是否符合SQL规则。
- 查询检查：根据数据字典对合法的查询语句进行语义检查。
- 查询优化：从多种可供选择的执行策略中选择一个高效执行的查询处理策略。
- 查询执行：代码生成器生成执行这个查询计划的代码。

选择操作（`select`）的查询算法实现：
- 简单全表扫描：顺序扫描基本表，逐一检查符合条件的元组。
- 索引（或散列）扫描方法：如果选择条件的属性上有索引（如B+树索引或Hash索引），可以用索引扫描方法，通过索引先找到满足条件的元组主码或元组指针，再通过元组指针直接在查询的基本表中找到元组
...
数据的不一致性问题：并发操作的错误调度可能导致数据不一致，分3种情形：
- 丢失修改：两个事务写同一数据，则其中一个事务的数据会丢失
- 不可读重复：T1读后T2更新，则T1无法再现前一次的读取结果。
- 读脏数据：T1写入后T2读取，T2读取后T1撤销，则T2持有的是脏数据，即不正确的数据。

### 封锁

封锁：事务T在对某个数据对象操作之前，向系统申请对其加锁。加锁后事务T便对数据对象有了一定的控制，在释放锁之前，其他事务对该数据对象的操作受到限制。

**排它锁**（eXclusion lock，X锁，写锁）：事务T对A上X锁，则只允许T*读取和修改*A，其他任何事物不能给A加锁，直到T释放这个锁。

**共享锁**（Shared lock，S锁，读锁）：事务T对A上S锁，则事务T*可以读取但不能修改*A，其他事务可以读取A，也可以上S锁，但不能上X锁，直到A的所有S锁被释放。
...
**活锁**（livelock）：一个事务可能永久等待（系统总是先批准其他事务的请求），可以通过FCFS策略解决。

**死锁**（deadlock）：两个事务互相申请对方锁住的资源，导致两个事务永远不可能结束。

死锁的四个要件：不可剥夺、资源互斥、请求保持、循环等待

死锁的预防：
- 一次性封锁：每个事务一次性为所有数据加锁（可能会增大封锁范围，降低并发度）
- 顺序封锁法：对数据对象指定一个上锁次序，所有事务都遵循次序上锁（难以事先确定封锁顺序）

死锁的检测：银行家算法

死锁的解除：
- 超时法：事务等待超时则默认为死锁（可能会误判或者耽误时间）
- 等待图法：并发控制子系统周期性生成*事务等待图*，并不断检测图中的回路，如出现回路则表示发生了死锁。如果发生死锁，一般思路是撤销一个处理代价较小的事务，并回滚其对数据的修改。

![[Pasted image 20240206162908.png]]

### 并发调度的可串行性

**调度可串行化**（serializable scheduling）：多个事务的并发执行是正确的，当且仅当其结果与按某一次序串行地执行这些事务时的结果相同，即事务的并发调度是可串行化的。

**冲突可串行化调度**：
- *冲突操作*是指不同事务对同一个数据的读写操作和写写操作，如`Ri(x),Wj(x)`或`Wi(x),Wj(x)`。
- 不能交换的动作：
	- 同一事务的两个操作，如`Wj(x),Rj(x)`不能交换顺序。
	- 不同事务的冲突操作。
- *冲突可串行化*（conflict serializable）：一个调度`Sc`在保证冲突操作的次序不变的情况下，通过交换两个事务不冲突操作的次序得到另一个调度`Sc'` ，如果`Sc'`是串行的，称调度`Sc`为冲突可串行化的调度

调度正确<调度可串行化<调度冲突可串行化
...
**封锁协议**（locking protocol）：对数据对象加锁时约定的一些规则，如何时申请封锁、持锁时间、何时释放锁等。两段锁协议是最常用的一种封锁协议，使用两段锁协议产生的是可串行化调度（充分条件）。
