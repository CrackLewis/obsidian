...
在标签偏移情形中，标签分布$q(y)\neq p(y)$，但类别条件分布不变，即$q(x|y)=p(x|y)$。

如果源分布$q(y)$是“错误”的，可以用实际风险的恒等式修正：
$$
\begin{aligned}
\iint l(f(\mathbf{x}), y) p(\mathbf{x} \mid y)p(y) \;d\mathbf{x}dy =
\iint l(f(\mathbf{x}), y) q(\mathbf{x} \mid y)q(y)\frac{p(y)}{q(y)} \;d\mathbf{x}dy.
\end{aligned}
$$
这里，重要性权重将对应于标签似然比率：
$$
\beta_i\overset{def}= \dfrac{p(y_i)}{q(y_i)}
$$
为了估计目标标签分布，首先需要已经训练好的分类模型，其次用（来自训练分布的）测试集进行验证，计算得到混淆矩阵$C_{k\times k}$。其中$C_{ij}$表示验证集中，真实标签为$j$，而预测标签为$i$的样本所占比例。

由于真实环境下样本标签不可见，所以混淆矩阵不能直接计算，然而可以将所有模型的预测结果取平均值，得到平均输出$\mu(\hat{y})\in \mathbb{R}^k$，其中$\mu(\hat{y}_i)$是模型预测测试集中$i$的总预测分数。
